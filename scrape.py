...
class Scraper:
    ...
    def get_responses(
        ...
        i: int, 
        shard_size: int,
        model_settings: dict = {"max_tokens": 512},
        output_path: str = '',
        source: str = ''
    ):
        ...
        Args:
            ...
            model_settings (dict, optional): A dictionary of settings to pass to the OpenAIChat model. Defaults to {"max_tokens": 512}.
        ...
        # Patch: Enforce a reasonable upper bound for max_tokens to prevent resource exhaustion
        DEFAULT_MAX_TOKENS = 512
        MAX_ALLOWED_TOKENS = 2048

        # Defensive: ensure model_settings is a dict, not shared between calls
        settings = dict(model_settings) if isinstance(model_settings, dict) else {}
        ...
        for prompt in tqdm(prompts):
            ...
            with jsonlines.open(output_path, mode="a") as writer:
                try:
                    json_data = {"00": prompt, "01": output, "model_settings": settings, "source": source, "00_len": len(prompt), "01_len": len(output)}
