...
class Scraper:
    # Define safe upper bound for tokens generated by LLM per request
    MAX_SAFE_TOKENS = 512  # Adjustable upper bound (commonly 256 - 1024), prevents resource exhaustion
    ...
    def _validate_max_tokens(self, value):
        """
        Ensure max_tokens is between 1 and MAX_SAFE_TOKENS.
        """
        try:
            value_int = int(value)
        except Exception:
            return self.MAX_SAFE_TOKENS
        if value_int < 1:
            return 1
        if value_int > self.MAX_SAFE_TOKENS:
            return self.MAX_SAFE_TOKENS
        return value_int
    ...
    def get_responses(
        ...
        # Validate max_tokens parameter coming from model_settings or use MAX_SAFE_TOKENS
        requested_max_tokens = model_settings.get("max_tokens", self.MAX_SAFE_TOKENS)
        safe_max_tokens = self._validate_max_tokens(requested_max_tokens)

        # Update model_settings to reflect actual used max_tokens (in case it's used elsewhere)
        model_settings = dict(model_settings)
        model_settings["max_tokens"] = safe_max_tokens

        model = OpenAIChat(
            model_name="gpt-3.5-turbo",
            openai_api_key=self.openai_api_keys[random.randint(0, len(self.openai_api_keys) - 1)],
            model_kwargs={"max_tokens": safe_max_tokens},  # Safe capped value
        )
